...
export async function GetArticlesCategory(category) {
      try {
        await connect();
        const newcategory = await Category.findOne({category: category}).sort({publishedAt: -1}).populate('articles')
...
export async function fetchLatestCategory(category)
  {
    ...
    try
    {
      const latest = await Article.findOne({category}).sort({publishedAt: -1});
      console.log(latest);
      return latest;
    }
    catch (err)
